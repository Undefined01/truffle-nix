...
public abstract class AttrsetLiteralNode extends NixNode {
  ...
  public AttrsetLiteralNode(List<Pair<String, NixNode>> attrsetElementExprs) {
    ...
    this.attrsetPropertyNames = attrsetElementExprs.stream().map(Pair::getLeft).toArray(String[]::new);
    this.attrsetElementNode = attrsetElementExprs.stream().map(Pair::getRight).toArray(NixNode[]::new);
  }
  ...
  public Object doGeneric(VirtualFrame frame,
   @CachedLibrary(limit = "3") DynamicObjectLibrary objectLibrary) {
    ...
    for (var i = 0; i < this.attrsetPropertyNames.length; i++) {
      objectLibrary.put(setObj, this.attrsetPropertyNames[i], this.attrsetElementNode[i].executeGeneric(frame));
    }
    return setObj;
  }
}
